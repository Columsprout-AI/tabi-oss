...
- Selects a **random batch excluding the final document**, because ingestion’s last batch may contain fewer than 10 rows. *(If your use case prefers including it, remove that exclusion in code.)*
...
- If you use a system prompt, make it explicit:  
...
```bash
curl -X POST http://localhost:5002/api/exp/start-experiment \
...
- **Preview shows fewer rows than expected** → By design, the last (possibly short) batch is excluded from random selection. If you want it included, change the selection logic.
