...
# MARKUP_CONSTANT=1.2      # used only by /estimate-credits
```
...
- `estimatedCredits = lastExpCredits * documentCount * MARKUP_CONSTANT` *(default 1.2, used only in estimate step).*
...
- Totals are converted to **credits** *(same rule as experiment: $0.01 ≈ 100 credits).*
...
**Estimate credits**
```bash
curl -X POST http://localhost:5003/api/proc/estimate-credits   -H 'Content-Type: application/json'   -d '{
...
**Start processing**
```bash
curl -X POST http://localhost:5003/api/proc/start-processing   -H 'Content-Type: application/json'   -d '{
...
- **Throughput** → loop is intentionally sequential to avoid race conditions. For large files, consider parallelization.  
- **Export linkage** → ingestion service later reads the updated batch docs via `/output-path` to assemble the final CSV.  
V