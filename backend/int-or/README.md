...
- FE listeners (post-signup/login/logout) trigger these calls to int-or (after your auth UI completes):
...
{ "sessionId": "<clerk-session-id>", "expPrompt": "<your prompt>" }
```
*(InputColumn isn’t required here; int-or resolves userId/fileId/inputIndex from the session and forwards { userId, fileId, inputIndex, expPrompt, experimentId } to the experiment service.)*
